...
@restock_orders.route('/', methods=['GET'])
...
@restock_orders.route('/<int:restock_order_id>', methods=['GET'])
...
# Get details for a specific restock order
@restock_orders.route('/<int:restock_order_id>/items', methods=['GET'])
def get_restock_order_items(restock_order_id):
    cursor = db.get_db().cursor()
    cursor.execute('SELECT * FROM RestockOrderItems WHERE RestockOrderID = %s', (restock_order_id,))
    row_headers = [x[0] for x in cursor.description]
    result = cursor.fetchone()
    if result:
        json_data = dict(zip(row_headers, result))
...
@restock_orders.route('/', methods=['POST'])
...
@restock_orders.route('/<int:restock_order_id>', methods=['PUT'])
...
@restock_orders.route('/<int:restock_order_id>', methods=['DELETE'])
